...
include { DEEPLOC2 } from '/dados04/matheus/rgaprofiler/omatheuspimenta-rgaprofiler/modules/local/deeploc2/tests/../main.nf'
...
workflow {
    ...
    if (DEEPLOC2.output){
        ...
        DEEPLOC2.out.getNames().each { name ->
            serializeChannel(name, DEEPLOC2.out.getProperty(name), jsonOutput, params.nf_test_output)
        }	  

        // consumes all unnamed output channels and stores items in a json file
        def array = DEEPLOC2.out as List<Object>
        def i = 0
        array.each { output ->
