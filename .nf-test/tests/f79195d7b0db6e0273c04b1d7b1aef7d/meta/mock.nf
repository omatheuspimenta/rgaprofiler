...
include { FASTA_QC } from '/dados04/matheus/rgaprofiler/omatheuspimenta-rgaprofiler/modules/local/fasta_qc/tests/../main.nf'
...
workflow {
    ...
    //run process
    FASTA_QC.run(input.toArray())

    if (FASTA_QC.output){

        // consumes all named output channels and stores items in a json file
        FASTA_QC.out.getNames().each { name ->
            serializeChannel(name, FASTA_QC.out.getProperty(name), jsonOutput, params.nf_test_output)
        }	  

        // consumes all unnamed output channels and stores items in a json file
        def array = FASTA_QC.out as List<Object>
        def i = 0
        array.each { output ->
