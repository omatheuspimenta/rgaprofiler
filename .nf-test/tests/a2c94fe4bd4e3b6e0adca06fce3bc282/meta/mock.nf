...
include { SIGNALP6 } from '/dados04/matheus/rgaprofiler/omatheuspimenta-rgaprofiler/modules/local/signalp6/tests/../main.nf'
...
workflow {
    ...
    if (SIGNALP6.output){
        ...
        SIGNALP6.out.getNames().each { name ->
            serializeChannel(name, SIGNALP6.out.getProperty(name), jsonOutput, params.nf_test_output)
        }	  

        // consumes all unnamed output channels and stores items in a json file
        def array = SIGNALP6.out as List<Object>
        def i = 0
        array.each { output ->
