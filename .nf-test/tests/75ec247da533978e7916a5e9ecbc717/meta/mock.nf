...
include { PHOBIUS } from '/dados04/matheus/rgaprofiler/omatheuspimenta-rgaprofiler/modules/local/phobius/tests/../main.nf'
...
workflow {
    ...
    if (PHOBIUS.output){
        ...
        PHOBIUS.out.getNames().each { name ->
            serializeChannel(name, PHOBIUS.out.getProperty(name), jsonOutput, params.nf_test_output)
        }	  

        // consumes all unnamed output channels and stores items in a json file
        def array = PHOBIUS.out as List<Object>
        def i = 0
        array.each { output ->
