...
include { DEEPTMHMM } from '/dados04/matheus/rgaprofiler/omatheuspimenta-rgaprofiler/modules/local/deeptmhmm/tests/../main.nf'
...
workflow {
    ...
    if (DEEPTMHMM.output){
        ...
        DEEPTMHMM.out.getNames().each { name ->
            serializeChannel(name, DEEPTMHMM.out.getProperty(name), jsonOutput, params.nf_test_output)
        }	  

        // consumes all unnamed output channels and stores items in a json file
        def array = DEEPTMHMM.out as List<Object>
        def i = 0
        array.each { output ->
